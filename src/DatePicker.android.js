...
export default class DatePicker extends React.Component<Props, State> {
  constructor(props: Props) {
    ...
    const selectedDate = this.props.initDate ? new Date(this.props.initDate) : new Date();
    ...
    let daysStartDate = 0;
    if (startDate) {
      millisBetweenStartDate = new Date(startDate).getTime() - new Date().getTime();
      daysStartDate = millisBetweenStartDate / millisecondsPerDay;
    }
    const days = millisBetween / millisecondsPerDay;
    const daysAfterSelectedDate = Math.round(daysStartDate);
    const initDayInex = startDate ? Math.round(days) - Math.round(daysStartDate) : Math.round(days);
    const initHourInex = this.props.format24 ? time24format : Number(time12format[0]) - 1;
